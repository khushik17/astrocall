    ...
    <div className="min-h-screen flex items-center justify-center bg-[#06030f] px-4 relative overflow-hidden">
      ...
      <div className="relative flex flex-col items-center gap-6 sm:gap-8 max-w-sm w-full text-center z-10">
        <div className="relative flex items-center justify-center pt-8 mb-2">
          <div className="absolute w-44 h-44 sm:w-52 sm:h-52 rounded-full border border-gold-400/20" style={{ animation: "radarPing 3s cubic-bezier(0,0,0.2,1) infinite" }} />
          <div className="absolute w-32 h-32 sm:w-40 sm:h-40 rounded-full border border-purple-400/30" style={{ animation: "radarPing 3s cubic-bezier(0,0,0.2,1) infinite 0.75s" }} />
          <div className="absolute w-24 h-24 sm:w-28 sm:h-28 rounded-full border-2 border-purple-400/40" style={{ animation: "radarPing 3s cubic-bezier(0,0,0.2,1) infinite 1.5s" }} />
        ...
        <div className="space-y-2 sm:space-y-3 px-2">
          ...
          <h2 className="font-display text-2xl sm:text-4xl text-white tracking-wide">Calling {session.astroName}</h2>
          <p className="font-body text-purple-300 italic text-sm sm:text-base">Signaling the cosmos… please hold</p>
        </div>
        <button onClick={handleCancel} disabled={cancelling} className="mt-4 sm:mt-6 flex flex-col items-center gap-2 group transition-all duration-300 disabled:opacity-50">
          <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300 border border-red-500/30" style={{ background: "linear-gradient(135deg,rgba(153,27,27,0.5),rgba(69,10,10,0.8))", boxShadow: "0 10px 20px -5px rgba(185,28,28,0.3)" }}>
      ...
      <style jsx>{`
        @keyframes radarPing { 0%{transform:scale(0.5);opacity:1;} 100%{transform:scale(1.3);opacity:0;} }
      `}</style>
    </div>
  );
  ...
  return (
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#05020e", overflow: "hidden", position: "relative" }}>
      <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
          ...
          <div style={{ flex: 1, position: "relative", background: "linear-gradient(135deg,#0d0520 0%,#11062b 100%)" }}>
            ...
            {loadingToken || !tokenData ? (
              <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, padding: "0 1rem" }}>
                <div style={{ width: "clamp(80px,20vw,120px)", height: "clamp(80px,20vw,120px)", borderRadius: "50%", background: "linear-gradient(135deg,rgba(124,58,237,0.3),rgba(245,158,11,0.1))", border: "2px solid rgba(124,58,237,0.4)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "clamp(30px,8vw,48px)" }}>🔮</div>
            ...
            {/* Name badge */}
            <div style={{ position: "absolute", top: 12, left: 12, zIndex: 10, display: "flex", alignItems: "center", gap: 8, background: "rgba(5,2,14,0.75)", backdropFilter: "blur(12px)", borderRadius: 10, border: "1px solid rgba(139,92,246,0.2)", padding: "6px 10px" }}>
            ...
            {/* Timer badge */}
            {session?.startedAt && (
              <div style={{ position: "absolute", top: 12, right: 12, zIndex: 10, display: "flex", alignItems: "center", gap: 5, background: "rgba(5,2,14,0.75)", backdropFilter: "blur(12px)", borderRadius: 8, border: "1px solid rgba(245,158,11,0.25)", padding: "5px 10px" }}>
        ...
        >
          <ChatPanel
            chatMsgs={chatMsgs} chatInput={chatInput}
            setChatInput={setChatInput} sendChat={sendChat}
            userId={user?.uid} onClose={() => setChatOpen(false)}
      ...
      <div className="sm:hidden" style={{ position: "fixed", inset: 0, zIndex: 60, pointerEvents: chatOpen ? "auto" : "none" }}>
        {chatOpen && <div onClick={() => setChatOpen(false)} style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.5)" }} />}
        <div style={{
      ...
      {!chatOpen && (
        <button onClick={() => setChatOpen(true)} style={{
          position: "fixed", right: 12, bottom: "clamp(80px,18vw,100px)", zIndex: 50,
          width: 40, height: 40, borderRadius: 10,
          background: "rgba(109,40,217,0.35)", border: "1px solid rgba(139,92,246,0.35)",
...
// ── Chat panel ────────────────────────────────────────────────────────────────
function ChatPanel({ chatMsgs, chatInput, setChatInput, sendChat, userId, onClose, chatBottomRef }: {
  chatMsgs: ChatMsg[];
  chatInput: string;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
      <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(139,92,246,0.15)", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 12px", display: "flex", flexDirection: "column", gap: 10 }}>
        {chatMsgs.length === 0 && (
          <div style={{ textAlign: "center", marginTop: 32 }}>
            ...
            <p style={{ fontFamily: "'Cormorant Garamond',serif", color: "#374151", fontSize: "0.85rem", fontStyle: "italic" }}>No messages yet.<br />Start the conversation!</p>
          </div>
        )}
        {chatMsgs.map(msg => {
          ...
          return (
            <div key={msg.id} style={{ display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start" }}>
              ...
              <div style={{ maxWidth: "80%", padding: "8px 12px", borderRadius: isMe ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: isMe ? "linear-gradient(135deg,rgba(124,58,237,0.4),rgba(109,40,217,0.3))" : "rgba(255,255,255,0.04)", border: isMe ? "1px solid rgba(139,92,246,0.35)" : "1px solid rgba(255,255,255,0.06)" }}>
                <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "0.9rem", color: isMe ? "#e9d5ff" : "#9ca3af", margin: 0, lineHeight: 1.5 }}>{msg.text}</p>
              </div>
            </div>
          );
        })}
      ...
      <div style={{ padding: "10px 12px", borderTop: "1px solid rgba(139,92,246,0.15)", display: "flex", gap: 7, flexShrink: 0 }}>
        <input
          ...
          style={{ flex: 1, background: "rgba(109,40,217,0.08)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 10, padding: "9px 12px", color: "#e9d5ff", outline: "none", fontFamily: "'Cormorant Garamond',serif", fontSize: "0.9rem", minWidth: 0 }}
        />
        <button onClick={sendChat} style={{ width: 36, height: 36, borderRadius: 10, border: "none", cursor: "pointer", background: "linear-gradient(135deg,#7c3aed,#6d28d9)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
